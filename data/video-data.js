const videoLibrary = [
  {
    videoId: "gnscqweM_NU",
    imageURL: "http://i3.ytimg.com/vi/gnscqweM_NU/maxresdefault.jpg",
    name: "Figma Tutorial: Shape Tools",
    description:
      "Learn How to create shapes in Figma, Selecting and resizing shapes using keyboard shortcuts, How to find and just shape properties, How to change shape fills",
    channelName: "Figma",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwngedleQSjBTUabDr5IuXLEaMLy6lSCFMPzBi00V5w=s88-c-k-c0x00ffffff-no-rj",
    topic: "Figma Basics",
  },

  {
    videoId: "5x2uHUB_pzw",
    imageURL: "http://i3.ytimg.com/vi/5x2uHUB_pzw/maxresdefault.jpg",
    name: "Figma Tutorial: Pen Tool Basics & Vector Networks",
    description:
      "In this video we'll learn how to use the pen tool and vector networks.",
    channelName: "Figma",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwngedleQSjBTUabDr5IuXLEaMLy6lSCFMPzBi00V5w=s88-c-k-c0x00ffffff-no-rj",
    topic: "Figma Basics",
  },

  {
    videoId: "NrKX46DzkGQ",
    imageURL: "http://i3.ytimg.com/vi/NrKX46DzkGQ/maxresdefault.jpg",
    name: "Figma Tutorial: Auto Layout (older version)",
    description:
      "With Auto Layout, buttons can resize with their text, lists can be rearranged by simply dragging items around, and your Frames will grow or shrink as you add or remove content. ",
    channelName: "Figma",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwngedleQSjBTUabDr5IuXLEaMLy6lSCFMPzBi00V5w=s88-c-k-c0x00ffffff-no-rj",
    topic: "Figma Basics",
  },

  {
    videoId: "LHY9cm_2zwU",
    imageURL: "http://i3.ytimg.com/vi/LHY9cm_2zwU/maxresdefault.jpg",
    name: "Figma Tutorial: Constraints",
    description: "Learn how you can leverage constraints in figma.",
    channelName: "Figma",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwngedleQSjBTUabDr5IuXLEaMLy6lSCFMPzBi00V5w=s88-c-k-c0x00ffffff-no-rj",
    topic: "Figma Basics",
  },

  {
    videoId: "gUb303HQSGk",
    imageURL: "http://i3.ytimg.com/vi/gUb303HQSGk/maxresdefault.jpg",
    name: "Figma Tutorial: Gradients (Linear, Radial, Angular, Diamond)",
    description:
      "In this video we'll learn how to use various types of gradients in our designs.",
    channelName: "Figma",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwngedleQSjBTUabDr5IuXLEaMLy6lSCFMPzBi00V5w=s88-c-k-c0x00ffffff-no-rj",
    topic: "Figma Basics",
  },

  {
    videoId: "e1BRH09iwes",
    imageURL: "http://i3.ytimg.com/vi/e1BRH09iwes/maxresdefault.jpg",
    name: "Figma Tutorial: Effects",
    description:
      "In this video we'll show you how to change the appearance of objects on your canvas by using Effects. ",
    channelName: "Figma",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwngedleQSjBTUabDr5IuXLEaMLy6lSCFMPzBi00V5w=s88-c-k-c0x00ffffff-no-rj",
    topic: "Figma Basics",
  },

  {
    videoId: "V7waqacFYZs",
    imageURL: "http://i3.ytimg.com/vi/V7waqacFYZs/maxresdefault.jpg",
    name: "Figma Tutorial: Color Picker",
    description:
      "In this video we'll talk about the Color Picker and how you can select and save colors in Figma.",
    channelName: "Figma",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwngedleQSjBTUabDr5IuXLEaMLy6lSCFMPzBi00V5w=s88-c-k-c0x00ffffff-no-rj",
    topic: "Figma Basics",
  },

  {
    videoId: "gtQ_A3imzsg",
    imageURL: "http://i3.ytimg.com/vi/gtQ_A3imzsg/maxresdefault.jpg",
    name: "Figma Tutorial: Creating Styles",
    description:
      "In this video we'll show you how to create Styles and use them across your designs.",
    channelName: "Figma",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwngedleQSjBTUabDr5IuXLEaMLy6lSCFMPzBi00V5w=s88-c-k-c0x00ffffff-no-rj",
    topic: "Figma Basics",
  },

  {
    videoId: "L7dtFGUd-C0",
    imageURL: "http://i3.ytimg.com/vi/L7dtFGUd-C0/maxresdefault.jpg",
    name: "Figma Tutorial: Rounded Corners",
    description:
      "In this video we'll show you how to soften the corners of rectangles, stars, or other polygons with Figma's corner rounding feature.",
    channelName: "Figma",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwngedleQSjBTUabDr5IuXLEaMLy6lSCFMPzBi00V5w=s88-c-k-c0x00ffffff-no-rj",
    topic: "Figma Basics",
  },

  {
    videoId: "5i-ebNTjad8",
    imageURL: "http://i3.ytimg.com/vi/5i-ebNTjad8/maxresdefault.jpg",
    name: "Figma Tutorial: Text Tool and Fonts",
    description:
      "In this video we’ll cover the basics of using the text tool and fonts..",
    channelName: "Figma",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwngedleQSjBTUabDr5IuXLEaMLy6lSCFMPzBi00V5w=s88-c-k-c0x00ffffff-no-rj",
    topic: "Figma Basics",
  },

  {
    videoId: "Dn0Ldi1SPv0",
    imageURL: "http://i3.ytimg.com/vi/Dn0Ldi1SPv0/maxresdefault.jpg",
    name: "Figma Tutorial: Masks",
    description:
      "In this video we'll learn how Masks can be used to show, and hide, specific portions of other layers on your canvas.",
    channelName: "Figma",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwngedleQSjBTUabDr5IuXLEaMLy6lSCFMPzBi00V5w=s88-c-k-c0x00ffffff-no-rj",
    topic: "Figma Basics",
  },

  {
    videoId: "8ZaX6qEcUCU",
    imageURL: "http://i3.ytimg.com/vi/8ZaX6qEcUCU/maxresdefault.jpg",
    name: "Figma Tutorial: Boolean Operations",
    description:
      "In this video we'll learn how to combine shapes using each of the four Boolean Operations: Union, Subtract, Intersect, and Exclude.",
    channelName: "Figma",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwngedleQSjBTUabDr5IuXLEaMLy6lSCFMPzBi00V5w=s88-c-k-c0x00ffffff-no-rj",
    topic: "Figma Basics",
  },

  {
    videoId: "saoB8uqUAH8",
    imageURL: "http://i3.ytimg.com/vi/saoB8uqUAH8/maxresdefault.jpg",
    name: "Figma Tutorial: imageURLs",
    description:
      "In this video we'll show you how to add and use imageURLs in your designs. ",
    channelName: "Figma",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwngedleQSjBTUabDr5IuXLEaMLy6lSCFMPzBi00V5w=s88-c-k-c0x00ffffff-no-rj",
    topic: "Figma Basics",
  },

  {
    videoId: "qQyaPqem0TA",
    imageURL: "http://i3.ytimg.com/vi/qQyaPqem0TA/maxresdefault.jpg",
    name: "Figma Tutorial: Alignment & Distribution",
    description:
      "In this video we'll learn how to align and distribute objects on our canvas.",
    channelName: "Figma",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwngedleQSjBTUabDr5IuXLEaMLy6lSCFMPzBi00V5w=s88-c-k-c0x00ffffff-no-rj",
    topic: "Figma Basics",
  },

  {
    videoId: "zd8wrAdURN0",
    imageURL: "http://i3.ytimg.com/vi/zd8wrAdURN0/maxresdefault.jpg",
    name: "Figma Tutorial: Layout Grids",
    description:
      "In this video we'll discuss the various types of Layout Grids you can use in Figma when working on responsive designs. ",
    channelName: "Figma",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwngedleQSjBTUabDr5IuXLEaMLy6lSCFMPzBi00V5w=s88-c-k-c0x00ffffff-no-rj",
    topic: "Figma Basics",
  },

  {
    videoId: "0xafhHNEsp8",
    imageURL: "http://i3.ytimg.com/vi/0xafhHNEsp8/maxresdefault.jpg",
    name: "Figma Tutorial: Guides",
    description:
      "In this video we'll discuss how to use Guides for aligning and measuring objects on the canvas.",
    channelName: "Figma",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwngedleQSjBTUabDr5IuXLEaMLy6lSCFMPzBi00V5w=s88-c-k-c0x00ffffff-no-rj",
    topic: "Figma Basics",
  },

  {
    videoId: "cAkOH9uaXHA",
    imageURL: "http://i3.ytimg.com/vi/cAkOH9uaXHA/maxresdefault.jpg",
    name: "Figma Tutorial: Easing Curves",
    description:
      "In this video we'll learn about easing curves and how to create your custom bezier easing curves.",
    channelName: "Figma",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwngedleQSjBTUabDr5IuXLEaMLy6lSCFMPzBi00V5w=s88-c-k-c0x00ffffff-no-rj",
    topic: "Figma Basics",
  },

  {
    videoId: "y29Xwt9dET0",
    imageURL: "http://i3.ytimg.com/vi/y29Xwt9dET0/maxresdefault.jpg",
    name: "Figma Tutorial: Variants",
    description:
      "Variants reduce the complexity of your design system, makes it easier to find components, and maps your design components more closely to code. In this video we'll teach you how to create and use variants and component sets",
    channelName: "Figma",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwngedleQSjBTUabDr5IuXLEaMLy6lSCFMPzBi00V5w=s88-c-k-c0x00ffffff-no-rj",
    topic: "Figma Basics",
  },

  {
    videoId: "YqQx75OPRa0",
    imageURL: "http://i3.ytimg.com/vi/YqQx75OPRa0/maxresdefault.jpg",
    name: "Beginning Graphic Design: Fundamentals",
    description:
      "In this video, you’ll learn the fundamentals of graphic design.",
    channelName: "GCFLearnFree.org",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwnhSc4rUizQ3LSxth3FxWehHQb8da2qs54dH5F9oIQ=s48-c-k-c0x00ffffff-no-rj",
    topic: "Design Fundamentals",
  },

  {
    videoId: "sByzHoiYFX0",
    imageURL: "http://i3.ytimg.com/vi/sByzHoiYFX0/maxresdefault.jpg",
    name: "Beginning Graphic Design: Typography",
    description:
      "In this video, learn more about the basics of working with typography.",
    channelName: "GCFLearnFree.org",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwnhSc4rUizQ3LSxth3FxWehHQb8da2qs54dH5F9oIQ=s48-c-k-c0x00ffffff-no-rj",
    topic: "Design Fundamentals",
  },

  {
    videoId: "_2LLXnUdUIc",
    imageURL: "http://i3.ytimg.com/vi/_2LLXnUdUIc/maxresdefault.jpg",
    name: "Beginning Graphic Design: Color",
    description:
      "In this video, you’ll learn the basics of using color in graphic design.",
    channelName: "GCFLearnFree.org",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwnhSc4rUizQ3LSxth3FxWehHQb8da2qs54dH5F9oIQ=s48-c-k-c0x00ffffff-no-rj",
    topic: "Design Fundamentals",
  },

  {
    videoId: "a5KYlHNKQB8",
    imageURL: "http://i3.ytimg.com/vi/a5KYlHNKQB8/maxresdefault.jpg",
    name: "Beginning Graphic Design: Layout & Composition",
    description:
      "In this video, you’ll learn the basics of layout and composition in graphic design.",
    channelName: "GCFLearnFree.org",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwnhSc4rUizQ3LSxth3FxWehHQb8da2qs54dH5F9oIQ=s48-c-k-c0x00ffffff-no-rj",
    topic: "Design Fundamentals",
  },

  {
    videoId: "MELKuexR3sQ",
    imageURL: "http://i3.ytimg.com/vi/MELKuexR3sQ/maxresdefault.jpg",
    name: "Beginning Graphic Design: imageURLs",
    description:
      "In this video, you’ll learn the basics of using imageURLs in graphic design.",
    channelName: "GCFLearnFree.org",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwnhSc4rUizQ3LSxth3FxWehHQb8da2qs54dH5F9oIQ=s48-c-k-c0x00ffffff-no-rj",
    topic: "Design Fundamentals",
  },

  {
    videoId: "l-S2Y3SF3mM",
    imageURL: "http://i3.ytimg.com/vi/l-S2Y3SF3mM/maxresdefault.jpg",
    name: "Beginning Graphic Design: Branding & Identity",
    description:
      "In this video, you’ll learn the basics of using branding and identity in graphic design.",
    channelName: "GCFLearnFree.org",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwnhSc4rUizQ3LSxth3FxWehHQb8da2qs54dH5F9oIQ=s48-c-k-c0x00ffffff-no-rj",
    topic: "Design Fundamentals",
  },
  {
    videoId: "1Dh8ZBQp9jo",
    imageURL: "http://i3.ytimg.com/vi/1Dh8ZBQp9jo/maxresdefault.jpg",
    name: "The New Material.io",
    description:
      "Discover what’s new with Material.io with a walk through of the new Design, Develop, and Tools landing pages. Learn how to access our Tools and Material Studies available to designers and developers.",
    channelName: "Google Design",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwnjps1VS0yQEw1TjW3xk19K-SRgCw2qT8O1eZKP4kw=s48-c-k-c0x00ffffff-no-rj",
    topic: "Material Design",
  },
  {
    videoId: "AUXKtt6bizw",
    imageURL: "http://i3.ytimg.com/vi/AUXKtt6bizw/maxresdefault.jpg",
    name: "Understanding the Material Design type system | Google Design Tutorials",
    description:
      "In this tutorial, learn how typography works in Material Design, and create a custom type system with expressive headline styles, readable body styles, and everything in between.",
    channelName: "Google Design",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwnjps1VS0yQEw1TjW3xk19K-SRgCw2qT8O1eZKP4kw=s48-c-k-c0x00ffffff-no-rj",
    topic: "Material Design",
  },
  {
    videoId: "TB3L1LbC4zw",
    imageURL: "http://i3.ytimg.com/vi/TB3L1LbC4zw/maxresdefault.jpg",
    name: "Understanding the Material Design color system | Google Design Tutorials",
    description:
      "In this tutorial, learn how color works in Material, walking through primary and secondary colors, tonal palettes, “on” colors, and additional customization to express your brand in Material Design with a consistent, comprehensive approach to color.",
    channelName: "Google Design",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwnjps1VS0yQEw1TjW3xk19K-SRgCw2qT8O1eZKP4kw=s48-c-k-c0x00ffffff-no-rj",
    topic: "Material Design",
  },
  {
    videoId: "cnlabMHshcE",
    imageURL: "http://i3.ytimg.com/vi/cnlabMHshcE/maxresdefault.jpg",
    name: "Beginner Tips for Typography | Material Live Highlights",
    description: "How how to use Type effectively",
    channelName: "Material Design",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwnjL2xnFFmomBWHqO-jdgicDyX51RzVtzPq4Lq0l=s48-c-k-c0x00ffffff-no-rj",
    topic: "Material Design",
  },
  {
    videoId: "eXcKOqviLE0",
    imageURL: "http://i3.ytimg.com/vi/eXcKOqviLE0/maxresdefault.jpg",
    name: "How to Apply a Color Palette to Your Design – Tutorial",
    description:
      "How do I use a color palette? How many colors should I use? How will I know if my colors look good together? Making a color palette is the first step, but knowing how to use it is something else.",
    channelName: "The Futur Academy",
    channelLogoURL:
      "https://yt3.ggpht.com/ytc/AAUvwnipSvnTzICFTps8lEi_RY0Eg7e5ymSPVJpjN12g=s48-c-k-c0x00ffffff-no-rj",
    topic: "Color Theory",
  },
];

module.exports = { videoLibrary };
